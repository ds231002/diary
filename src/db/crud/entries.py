...
from db.crud.embeddings import generate_embedding
...
def update_entry(
    ...
    *,
    ...
    mood: Optional[int] = _UNSET,
    llm_allowed: Optional[bool] = _UNSET,
    tag_ids: Optional[list[UUID]] = _UNSET,   # 👈 neu ergänzt
) -> dict:
    
    # TODO function for get_entry(id)
    # TODO new_content != old_content
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                """
                SELECT content, llm_allowed, embedding
                FROM entries
            ...
            old_llm_allowed = current["llm_allowed"]
            old_embedding = current["embedding"]

            # 🔹 neuen Zustand bestimmen
            new_content = old_content if content is _UNSET else content
            new_llm_allowed = (
                old_llm_allowed if llm_allowed is _UNSET else llm_allowed
            )

            new_embedding = old_embedding

            # 🔹 Embedding-Logik
            if new_llm_allowed is False:
                new_embedding = None

            elif new_llm_allowed is True:
                content_changed = new_content != old_content
                embedding_missing = old_embedding is None

                if content_changed or embedding_missing:
                    try:
            ...
            if llm_allowed is not _UNSET:
                fields.append("llm_allowed = %s")
                values.append(new_llm_allowed)

            if new_embedding != old_embedding:
                fields.append("embedding = %s")
                values.append(new_embedding)

...
tab_tags, tab_other = st.tabs(["Tags", "Weitere Einstellungen"])
        ...
        for tag in tags:
            ...
            with st.container(border=True):
                ...
                if st.session_state.get(f"edit_tag_{tag['id']}"):
                    ...
                    #     "Position",
                    #     value=tag.get("position") or 0,
                    #     step=1,
                    #     key=f"position_input_{tag['id']}"
                    # )
                ...
                if st.session_state.get(
                    f"confirm_delete_tag_{tag['id']}"
                ):

                    st.error(
                        "⚠️ Dieser Tag wird dauerhaft gelöscht."
                    )
                    ...
                    with c2:
                        if st.button(
                            "Abbrechen",
                            key=f"cancel_delete_tag_{tag['id']}"
                        ):
                            st.session_state.pop(
                                f"confirm_delete_tag_{tag['id']}",
                                None
...
# ==================================================
# ZWEITER REITER (ERSATZ)
# ==================================================

with tab_other:
    st.subheader("Weitere Einstellungen")
    st.info("Hier können später weitere Optionen ergänzt werden.")
